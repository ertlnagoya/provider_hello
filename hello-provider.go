...
import (
	...
	hello "github.com/ertlnagoya/proto_hello"
...
var (
	fmsrv           = flag.String("fmsrv", "wss://fm.synergic.mobi:8443/", "HelloManager Server")
	hostName        = flag.String("host", "", "hello provider host name")
...
//	for _, v := range bmap["vehicles"].([]interface{}) {
		...
		hello := hello.HelloMessage{
			HelloMessage: string(m["hello_message"].(string)),
		}

		out, err := proto.Marshal(&hello)
		if err == nil {
...
func publishSupplyFromHelloManager(client *sxutil.SXServiceClient, ch chan error) {
	// Connect by SocketIO
	...
	sioClient.On(gosocketio.OnConnection, func(c *gosocketio.Channel, param interface{}) {
		log.Printf("Hello-Provider socket.io connected %v", c)
	})
	sioClient.On(gosocketio.OnDisconnection, func(c *gosocketio.Channel, param interface{}) {
		log.Printf("Hello-Provider socket.io disconnected %v", c)
...
func runPublishSupplyInfinite(sclient *sxutil.SXServiceClient) {
	ch := make(chan error)
	for {
		publishSupplyFromHelloManager(sclient, ch)
...
func main() {
	log.Printf("HelloProvider(%s) built %s sha1 %s", sxutil.GitVer, sxutil.BuildTime, sxutil.Sha1Ver)
	flag.Parse()
	...
	argJson := fmt.Sprintf("{Client:Hello}")
